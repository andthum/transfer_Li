...
information() {
    echo "Submit MDTools analysis scripts for each transferred lithium ion to"
    ...
    echo "'submit_mdt_analyses_lintf2_ether.py'"
    echo "(https://hpcss.readthedocs.io/en/latest/doc_pages/_sphinx_autosummary_analysis/submit_mdt_analyses_lintf2_ether.html)."
}
...
usage() {
    ...
    echo "        'submit_mdt_analyses_lintf2_ether.py' for possible options."
    echo "        Default: ${ana_scripts}."
    echo "  -f    Additional options (besides --system, --settings and"
    echo "        --scripts) to parse to"
    echo "        'submit_mdt_analyses_lintf2_ether.py' provided as one long,"
